...
type JournalView = "accounts" | "journal" | "inbox" | "shortcuts" | "shopee";
...
export function MonthlyJournalWorkspace({ initialView = "accounts" }: { initialView?: JournalView }) {
  ...
  return <section className={styles.workspace}>
    <header className={styles.header}>
      ...
      <button className={styles.refresh} type="button" onClick={() => void loadData()}>{loading ? "Loading…" : "Refresh"}</button>
    </header>
    <nav className={styles.nav} aria-label="Monthly Journal navigation">
      {([ ["accounts", "Chart of Accounts"], ["journal", "General Journal"], ["inbox", "Bank Statement Inbox"], ["shortcuts", "Shortcuts"], ["shopee", "Shopee PayLater"] ] as [JournalView, string][]).map(([key, label]) => <button key={key} type="button" className={view === key ? styles.activeNav : ""} onClick={() => setView(key)}>{label}</button>)}
    </nav>
    ...
    {view === "journal" && <div className={styles.twoColumn}>
    ...
    {view !== "accounts" && view !== "journal" && <section className={styles.card}><p className={styles.eyebrow}>MONTHLY JOURNAL</p><h2>{view === "inbox" ? "Bank Statement Inbox" : view === "shortcuts" ? "Posting Shortcuts" : "Shopee PayLater"}</h2><p>This is part of the new, separate Monthly Journal workspace. Set up your Chart of Accounts first; the bank-import, shortcut, and Shopee workflows will use these new accounts and will not touch the existing bookkeeping system.</p><button className={styles.primary} type="button" onClick={() => setView("accounts")}>Set up Chart of Accounts</button></section>}
  </section>;
}
