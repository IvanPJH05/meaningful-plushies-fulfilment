...
export function MonthlyJournalWorkspace({ initialView = "accounts" }: { initialView?: JournalView }) {
  ...
  return <section className={styles.workspace}>
    ...
    {view === "general_journal" && <section className={styles.card}><div className={styles.cardHeading}><div><p className={styles.eyebrow}>ACCOUNTING DATE DRIVES REPORTS</p><h2>General Journal</h2><p>Debits are shown first, credits second, with the description below each entry.</p></div><strong>{entries.length} posted entries</strong></div><div className={styles.journalList}>{entries.length ? entries.map((entry) => <article className={styles.journalEntry} key={entry.id}><div className={styles.entryDate}><b>{entry.accounting_date}</b><span>Paid {entry.paid_date}</span></div><div><p>Debit {accountById.get(entry.debit_account_id ?? "")?.name ?? "Deleted account"}<strong>RM {entry.amount.toFixed(2)}</strong></p><p className={styles.credit}>Credit {entry.journal_note || (accountById.get(entry.credit_account_id ?? "")?.name ?? "Deleted account")}<strong>RM {entry.amount.toFixed(2)}</strong></p>{entry.description && <p className={styles.description}>{entry.description}</p>}</div></article>) : <p className={styles.muted}>Nothing posted yet. Start from the Bank Statement Inbox.</p>}</div></section>}
