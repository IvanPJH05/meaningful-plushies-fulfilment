...
export function MonthlyJournalWorkspace({ initialView = "accounts" }: { initialView?: JournalView }) {
  ...
  const [contraForm, setContraForm] = useState({ paidDate: today(), accountingDate: today(), from: "", to: "", amount: "", note: "", description: "" });
  ...
  const contraAccounts = useMemo(() => accounts.filter((account) => account.classification === "asset"), [accounts]);
  ...
  async function postContraEntry(event: FormEvent) {
    event.preventDefault();
    if (!supabase) return;
    const amount = Number(contraForm.amount);
    if (!contraForm.from || !contraForm.to || contraForm.from === contraForm.to || !Number.isFinite(amount) || amount <= 0) return setMessage("Choose two different asset accounts and enter an amount.");
    const fromName = accountById.get(contraForm.from)?.name ?? "source account";
    const toName = accountById.get(contraForm.to)?.name ?? "destination account";
    const { error } = await supabase.from("monthly_journal_entries").insert({
      paid_date: contraForm.paidDate,
      accounting_date: contraForm.accountingDate,
      bank: "",
      bank_reference: "",
      journal_note: contraForm.note.trim() || `Contra transfer from ${fromName} to ${toName}`,
      description: contraForm.description.trim(),
      debit_account_id: contraForm.to,
      credit_account_id: contraForm.from,
      amount,
      source: "manual",
      status: "posted",
      entry_lines: [{ account_id: contraForm.to, debit: amount, credit: 0 }, { account_id: contraForm.from, debit: 0, credit: amount }],
    });
    if (error) return setMessage(error.message);
    setContraForm({ paidDate: today(), accountingDate: today(), from: "", to: "", amount: "", note: "", description: "" });
    setMessage(`Contra entry posted: ${fromName} to ${toName}.`);
    await loadData();
  }
