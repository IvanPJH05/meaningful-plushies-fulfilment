...
export function ManualOrderIntakeRecords({ sessionToken }: { sessionToken: string }) {
  ...
  function submissionRows(rows: ManualOrderIntake[], emptyText: string) {
    ...
    return rows.map((intake) => <tr key={intake.id}><td><strong>{manualOrderIntakeReference(intake.id)}</strong></td><td>{new Date(intake.createdAt).toLocaleString()}</td><td><strong>{intake.customerName}</strong><small>{intake.phoneOriginal}{intake.customerEmail ? ` · ${intake.customerEmail}` : ""}</small></td><td>{intake.character} · {intake.productDisplayName.match(/(\d+) seconds/i)?.[1] || ""}s</td><td>{intake.shippingAddress.address1}, {intake.shippingAddress.city}, {intake.shippingAddress.province} {intake.shippingAddress.zip}</td><td><span className={`manual-order-status ${intake.status === "created" ? "used" : "active"}`}>{intake.status === "created" ? "Paid" : intake.status === "ready_to_create" ? "Receipt attached" : "Awaiting payment"}</span></td><td>{intake.status === "created" ? <strong>{intake.shopifyOrderName || "Created"}</strong> : <label className={`manual-order-receipt-drop${draggingReceiptFor === intake.id ? " is-dragging" : ""}${busy === intake.id ? " is-busy" : ""}`} onDragOver={(event) => { event.preventDefault(); if (busy !== intake.id) setDraggingReceiptFor(intake.id); }} onDragLeave={() => setDraggingReceiptFor("")} onDrop={(event) => { event.preventDefault(); setDraggingReceiptFor(""); if (busy !== intake.id) void uploadAndCreate(intake, Array.from(event.dataTransfer.files || [])); }}><input hidden type="file" multiple accept="application/pdf,image/png,image/jpeg,image/webp" disabled={busy === intake.id} onChange={(event) => { void uploadAndCreate(intake, Array.from(event.target.files || [])); event.target.value = ""; }} /><strong>{busy === intake.id ? "CREATING ORDER..." : "DROP RECEIPT OR OPEN FILES"}</strong></label>}</td></tr>);
  }
