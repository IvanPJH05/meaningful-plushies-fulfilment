...
export function ShopifyAppWorkspace({ sessionToken }: { sessionToken: string }) {
  ...
  async function openAdminPreview(id: string) { try { const data = await request("POST", { action: "create_admin_preview", certificateId: id }); const link = `${closerStorefrontUrl}?certificate=${encodeURIComponent(data.certificateId)}&adminPreview=${encodeURIComponent(data.previewToken)}`; window.open(link, "_blank", "noopener,noreferrer"); setNotice(`Admin preview opened for ${id}. It expires in 15 minutes and does not change the customer link.`); } catch (error) { setNotice(error instanceof Error ? error.message : "Could not open the admin preview."); } }
