...
export function CloserCustomerPage({ proxyPath = "" }: { proxyPath?: string }) {
  ...
  return <main className={styles.page} style={themedStyle}><div className={styles.scene}>
    ...
    {state.status === "unlinked" ? <section className={styles.unlinkedSpace}>
      ...
      {showPhotoPicker && <section className={`${styles.card} ${styles.photoPicker}`} role="dialog" aria-modal="true" aria-label="Add a shared photo"><button type="button" className={styles.closeButton} onClick={() => setShowPhotoPicker(false)} aria-label="Close">×</button><h1><CloserWordArt className={styles.photoPickerTitle} asset="add-a-photo" label="Add a photo" /></h1><div className={styles.photoButtons}><button className={styles.primaryButton} disabled={busy} onClick={() => cameraPhotoInput.current?.click()}><CloserWordArt className={styles.photoActionArt} asset="take-photo" label="Take photo" /></button><button className={styles.primaryButton} disabled={busy} onClick={() => galleryPhotoInput.current?.click()}><CloserWordArt className={styles.photoActionArt} asset="open-gallery" label="Open gallery" /></button></div></section>}
    </section>}
  </div></main>;
}
