...
export function MonthlyJournalInbox() {
  ...
  return <div className="monthly-inbox">
    ...
    <div className="mj-undo-posted"><span>Need to correct a posted transaction? Tick a posted row, then undo its posting here.</span><button type="button" disabled={postingShortcut || removingRows || !selected.some((id) => rows.find((row) => row.id === id)?.status === "posted")} onClick={() => void undoSelectedPostings()}>Undo selected posting</button>{filter.status !== "posted" && <button type="button" className="secondary" disabled={postingShortcut || removingRows} onClick={() => setFilter({ ...filter, status: "posted" })}>Show posted rows</button>}</div>
