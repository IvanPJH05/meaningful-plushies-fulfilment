...
export function MonthlyJournalInbox() {
  ...
  async function shortcut(name: string) {
    ...
    if (!selectedRows.length) return setNotice("Select bank rows first.");
    const account = accounts.find((item) => item.name === (name.includes("Shopify") ? "Software & Subscriptions" : "Sales"));
    if (!account) return setNotice("Create the shortcut account in Chart of Accounts first.");
    for (const row of selectedRows) {
      const accountingDate = name.includes("Shopify") ? monthEnd(row.paid_date) : row.paid_date;
      const description = name.includes("Shopify") ? `Shopify subscription for ${accountingDate.slice(0, 7)}` : `Credit sales for ${row.paid_date.slice(0, 7)}`;
      await post({ ...row, accounting_date: accountingDate }, { account: account.id, classification: account.classification, note: description, description });
    }
    setNotice(`${selectedRows.length} row(s) posted with ${name}.`);
  }
  ...
  return <div className="monthly-inbox">
    ...
    <div className="mj-shortcuts"><b>{selected.length} selected unposted row(s)</b><button onClick={() => void shortcut("Credit sales")}>Credit sales</button><button onClick={() => void shortcut("Shopify subscription - previous month")}>Shopify subscription - previous month</button>{customShortcuts.map((shortcut) => <button key={shortcut.id} onClick={() => void applyCustomShortcut(shortcut)}>{shortcut.name}</button>)}<button onClick={() => setSelected([])}>Clear selection</button></div>
