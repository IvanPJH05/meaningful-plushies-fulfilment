...
create table if not exists public.preorders (
  id text primary key, customer_name text not null, phone text not null, character text not null, product_key text not null,
  shipping_region text not null check (shipping_region in ('WEST','EAST')), total_amount numeric not null, deposit_amount numeric not null default 0,
  balance_due_date date, status text not null default 'deposit_paid' check (status in ('deposit_paid','paid','cancelled')),
  payment_receipts jsonb not null default '[]'::jsonb, manual_order_id text, created_at timestamptz not null default now(), updated_at timestamptz not null default now(), paid_at timestamptz
);
