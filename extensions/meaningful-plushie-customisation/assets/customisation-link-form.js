(() => {
  document.querySelectorAll("[data-customisation-link-block]").forEach((block) => {
    ...
    calendar.addEventListener("click",(event)=>{const target=event.target.closest("button");if(!target)return;if(target.dataset.yearToggle!==undefined){calendar.querySelector(".mp-deferred-customisation__calendar-years").hidden=false;return;}if(target.dataset.month){month.setMonth(month.getMonth()+Number(target.dataset.month));render();return;}if(target.dataset.calendarYear){month.setFullYear(Number(target.dataset.calendarYear));render();return;}if(target.dataset.day){dateBox.value=format(new Date(month.getFullYear(),month.getMonth(),Number(target.dataset.day)));calendar.hidden=true;}});
