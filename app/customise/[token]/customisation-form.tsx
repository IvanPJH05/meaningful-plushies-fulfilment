...
export function CustomisationForm({ token }: { token: string }) {
  ...
  return <main className="customise-page"><form className="customise-card" onSubmit={submit}>
    ...
    {fields.map((field) => <label key={field.key}>{field.label}<input required type={field.type || "text"} value={form[field.key]} placeholder={field.placeholder} onChange={(event) => change(field.key, event.target.value)} /></label>)}
