...
export async function GET(request: Request) {
  try {
    ...
    // This is the number of individual attachment records included in the export,
    // rather than merely the number of messages that contain an attachment.
    const [totalConversations, totalMessages, totalAttachments] = await Promise.all([prisma.conversation.count({ where }), prisma.message.count({ where: { businessId: business.id, conversation: where } }), prisma.messageAttachment.count({ where: { message: { conversation: where } } })]);
