...
export default function Home() {
  ...
  return <main className="app-shell">
    <aside className="side-nav">
      ...
      <nav>
        <button className={view === "orders" ? "active" : ""} onClick={() => setView("orders")}><b>â–¦</b> Orders</button>
        <button className={view === "fulfilment" ? "active" : ""} onClick={() => setView("fulfilment")}><b>â‰¡</b> Fulfilment</button>
        <button className={view === "packing_slips" ? "active" : ""} onClick={() => setView("packing_slips")}><b>â–¤</b> Packing Slips</button>
        {session.role === "admin" && <button className={view === "import" ? "active" : ""} onClick={() => setView("import")}><b>â‡§</b> CSV Import</button>}
        <button className={view === "fulfilled" ? "active" : ""} onClick={() => setView("fulfilled")}><b>âœ“</b> Shipped</button>
      </nav>
      <div className="user-card"><div className="avatar">{session.name.slice(0, 1)}</div><div><strong>{session.name}</strong><span>{session.role === "admin" ? "Administrator" : "Fulfilment staff"}</span></div><button title="Sign out" onClick={() => setSession(null)}>â†ª</button></div>
    </aside>
    ...
    <section className="main-area">
      ...
      {notice && <div className="notice"><span>{notice}</span><button onClick={() => setNotice("")}>Ã—</button></div>}
...
function OrderDrawer({ order, role, actor, onClose, onUpdate, onStatus }: { order: Order; role: UserRole; actor: string; onClose: () => void; onUpdate: (patch: Partial<Order>) => void; onStatus: (status: OrderStatus) => void }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="order-drawer"><div className="drawer-header"><div><p>ORDER DETAIL</p><h2>{orderLabel(order)}</h2></div><button onClick={onClose}>Ã—</button></div><div className="drawer-body">
    ...
    <section className="detail-section"><h3>Status history</h3><div className="history">{[...order.statusHistory].reverse().map((event) => <div key={event.id}><span></span><div><strong>{statusLabels[event.status]}</strong><p>{event.changedBy} Â· {formatDate(event.changedAt, true)}</p>{event.note && <small>{event.note}</small>}</div></div>)}</div></section>
