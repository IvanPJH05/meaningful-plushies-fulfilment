...
const workspaceDefaultViews: Record<Workspace, View> = {
  ...
  monthly_journal: "monthly_journal_inbox",
...
const monthlyJournalNavItems: NavItem[] = [
  { view: "monthly_journal_inbox", label: "Bank Statement Inbox", icon: "ledger" },
  { view: "monthly_journal_import", label: "Import PDF Statement", icon: "import" },
  { view: "monthly_journal_shopee", label: "Shopee PayLater", icon: "cash" },
...
function tikTokAttachmentNeedsReupload(order: Order) {
  return order.salesChannel === "tiktok" && Boolean(order.tikTokFileName) && !order.tikTokFileDataUrl;
}
...
function isCodFulfilmentOrder(order: Order, manualOrders: ManualOrder[]) {
  // TikTok Shop collects marketplace payments itself. COD collection only
  // applies to orders that came through the WhatsApp/manual-order flow.
  if (fulfilmentSource(order, manualOrders) !== "whatsapp") return false;
...
export default function Home() {
  ...
  const [stagedBankStatementLineId, setStagedBankStatementLineId] = useState("");
  ...
  function ledgerPreview(transactionId = "preview", accountOverride?: AccountingCategory | null): AccountingLedgerEntry[] {
    ...
    if (event.value === "payment_processor_paid") {
      ...
      if (transactionForm.categoryId === "Stripe" || transactionForm.categoryId === "Xendit" || transactionForm.categoryId === "TikTok") {
        return [
  ...
  function bankStatementLedgerEntries(line: AccountingBankStatementLine, form: BankStatementMatchForm, account: AccountingCategory, transactionId: string, createdAt: string): AccountingLedgerEntry[] {
    ...
    if (form.businessEvent === "payment_processor_paid") {
      ...
      if (accountName === "Stripe" || accountName === "Xendit" || accountName === "TikTok") return [
  ...
  function bankStatementLineBankName(line: AccountingBankStatementLine) {
  ...
  function bankStatementLineWithMatches(line: AccountingBankStatementLine, transactionIds: string[], transactionsList: Pick<AccountingTransaction, "id" | "amount">[] = accountingTransactions): AccountingBankStatementLine {
  ...
  function findInternalTransferPair(line: AccountingBankStatementLine) {
    return internalTransferPairCandidates(line)[0] ?? null;
  }

  function internalTransferPairCandidates(line: AccountingBankStatementLine) {
    const amount = bankStatementLineAmount(line);
    const lineDate = Date.parse(line.transactionDate);
    if (!amount || Number.isNaN(lineDate)) return [];
    return bankStatementLines.filter((candidate) => {
      ...
      const closeDate = !Number.isNaN(candidateDate) && Math.abs(candidateDate - lineDate) <= 1 * 24 * 60 * 60 * 1000;
      return oppositeDirection && sameAmount && closeDate;
    }).sort((a, b) => Math.abs(Date.parse(a.transactionDate) - lineDate) - Math.abs(Date.parse(b.transactionDate) - lineDate) || a.rowNumber - b.rowNumber);
  }

  async function pairInternalTransferLine(line: AccountingBankStatementLine, selectedPair?: AccountingBankStatementLine) {
    const amount = bankStatementLineAmount(line);
    ...
    try {
      const actor = session?.displayName ?? "Admin";
      const now = new Date().toISOString();
      const pair = selectedPair ?? findInternalTransferPair(line);
      const updatedLine: AccountingBankStatementLine = {
        ...line,
        matchStatus: "ignored",
        suggestedEvent: "internal_transfer",
        suggestedAccount: "Internal Bank Transfer",
        notes: pair ? `Internal transfer paired with ${bankStatementLineBankName(pair)} ${formatDate(pair.transactionDate)} row ${pair.rowNumber}.` : "Internal transfer excluded from books. Matching bank line not imported yet.",
        updatedAt: now,
      };
      ...
      if (pair) {
        updates.push({
          ...
          suggestedEvent: "internal_transfer",
          suggestedAccount: "Internal Bank Transfer",
          notes: `Internal transfer paired with ${bankStatementLineBankName(line)} ${formatDate(line.transactionDate)} row ${line.rowNumber}.`,
          updatedAt: now,
        });
      }
  ...
  async function matchBankStatementLine(line: AccountingBankStatementLine, formOverride: Partial<BankStatementMatchForm> = {}) {
    const form = { ...(bankStatementMatchForms[line.id] ?? { businessEvent: line.suggestedEvent || "expense", accountName: line.suggestedAccount || "", notes: "" }), ...formOverride };
  ...
  async function stageBankStatementLine(line: AccountingBankStatementLine, paymentMethod = "", formOverride: Partial<BankStatementMatchForm> = {}) {
    const form = { ...(bankStatementMatchForms[line.id] ?? { businessEvent: line.suggestedEvent || (line.moneyIn > 0 ? "other_income" : "expense"), accountName: line.suggestedAccount || "", notes: "" }), ...formOverride };
    if (!form.businessEvent || form.businessEvent === "ignore") return ignoreBankStatementLine(line);
    if (form.businessEvent === "internal_transfer") return pairInternalTransferLine(line);
    const processorOptions = ["Stripe", "Xendit", "TikTok"];
    const accountName = form.businessEvent === "payment_processor_paid"
      ? processorOptions.includes(form.accountName) ? form.accountName : processorOptions[0]
      : form.businessEvent === "operating_cost"
        ? prepaidOperatingCostAccountName
        : form.accountName || line.suggestedAccount;
    const updatedLine: AccountingBankStatementLine = {
      ...line,
      rawData: {
        ...(line.rawData ?? {}),
  ...
  function useBankStatementLineForTransaction(line: AccountingBankStatementLine) {
    ...
    setTransactionForm((current) => ({
      ...
      transactionDate: dateKey(line.transactionDate),
      description: line.description,
      ...
      notes: [line.notes, "Created from bank statement in-progress row"].filter(Boolean).join(" | "),
    }));
    setNotice("Bank row loaded into the form. Fill the missing details, then save the transaction.");
  }

  async function markStagedBankLineMatched(transactionIds: string[], createdTransactions: Pick<AccountingTransaction, "id" | "amount">[]) {
    if (!stagedBankStatementLineId || !transactionIds.length) return;
    const line = bankStatementLines.find((item) => item.id === stagedBankStatementLineId);
    if (!line) return;
    const updatedLine: AccountingBankStatementLine = {
      ...bankStatementLineWithMatches(line, transactionIds, createdTransactions),
      rawData: {
        ...(line.rawData ?? {}),
  ...
  async function updateAccountingTransactionAmountForBankMatch(transaction: AccountingTransaction, amount: number) {
    const now = new Date().toISOString();
    const oldEntries = accountingLedgerEntries.filter((entry) => entry.transactionId === transaction.id);
    const scale = transaction.amount > 0 ? amount / transaction.amount : 1;
    const updatedEntries = oldEntries.map((entry) => ({ ...entry, amount: Math.max(0, entry.amount * scale) }));
    const debit = updatedEntries.length ? updatedEntries.filter((entry) => entry.entryType === "debit").reduce((total, entry) => total + entry.amount, 0) : amount;
    const credit = updatedEntries.length ? updatedEntries.filter((entry) => entry.entryType === "credit").reduce((total, entry) => total + entry.amount, 0) : amount;
    const updatedTransaction: AccountingTransaction = {
      ...transaction,
      amount,
      debit,
      credit,
      depositAmount: transaction.paymentStatus === "paid_in_full" ? amount : Math.min(transaction.depositAmount || 0, amount),
      updatedAt: now,
    };
    await saveAccountingTransaction(updatedTransaction);
    if (updatedEntries.length) await saveAccountingLedgerEntries(transaction.id, updatedEntries);
    setAccountingTransactions((current) => current.map((item) => item.id === transaction.id ? updatedTransaction : item));
    if (updatedEntries.length) setAccountingLedgerEntries((current) => [...current.filter((entry) => entry.transactionId !== transaction.id), ...updatedEntries]);
    return updatedTransaction;
  }

  async function linkBankStatementLineToTransaction(line: AccountingBankStatementLine, transaction: AccountingTransaction, correctedAmount?: number) {
    if (!transaction) return setNotice("Choose a transaction to match.");
    const nextAmount = correctedAmount === undefined || Number.isNaN(correctedAmount) ? transaction.amount : correctedAmount;
    if (!Number.isFinite(nextAmount) || nextAmount <= 0) return setNotice("Enter a valid transaction amount.");
    setSavingAccounting(true);
    try {
      const amountChanged = Math.abs(nextAmount - transaction.amount) >= 0.005;
      const transactionToLink = amountChanged ? await updateAccountingTransactionAmountForBankMatch(transaction, nextAmount) : transaction;
      const existingNote = bankStatementNoteValue(line).trim();
      const currentIds = bankStatementMatchedTransactionIds(line);
      if (currentIds.includes(transactionToLink.id)) return setNotice("This transaction is already linked to this bank row.");
      const transactionList = accountingTransactions.map((item) => item.id === transactionToLink.id ? transactionToLink : item);
      const remainingBefore = bankStatementRemainingAmount(line, transactionList);
      if (transactionToLink.amount - remainingBefore > 0.01) return setNotice(`This transaction is more than the amount left. ${formatMoney(remainingBefore)} is left to match.`);
      const nextIds = [...currentIds, transactionToLink.id];
      const baseLine = bankStatementLineWithMatches(line, nextIds, transactionList);
      const remaining = bankStatementRemainingAmount(baseLine, transactionList);
      const matchNote = `Matched to existing transaction: ${transactionToLink.description}`;
      const updatedLine: AccountingBankStatementLine = {
        ...baseLine,
        suggestedEvent: transactionToLink.businessEvent,
        suggestedAccount: transactionToLink.accountName,
      ...
      await insertSharedActivity({
        id: crypto.randomUUID(),
        action: amountChanged ? "Bank transaction amount corrected and matched" : "Bank statement line matched to existing transaction",
        detail: `${line.description} matched to ${transactionToLink.description} (${formatMoney(transactionToLink.amount)}). ${formatMoney(remaining)} left.`,
        actor: session?.displayName ?? "Admin",
        createdAt: new Date().toISOString(),
      });
      setNotice(remaining > 0.01 ? `Transaction linked. ${formatMoney(remaining)} left to match.` : amountChanged ? "Amount updated and bank line fully matched." : "Bank line fully matched.");
    } catch (error) {
      setNotice(readableError(error, "Could not update and match this transaction."));
    } finally {
      setSavingAccounting(false);
    }
  }
  ...
  async function createManualAccountingTransaction() {
    ...
    try {
      ...
      if (event.value === "other_income") {
        ...
        const createdOtherIncomeTransactions: Pick<AccountingTransaction, "id" | "amount">[] = [];
        ...
        for (const line of otherIncomeLinesToSave) {
          ...
          await saveAccountingTransaction({
            id: lineId,
            source: documentId ? "document" : stagedBankStatementLineId ? "bank_statement" : "manual",
            sourceId: documentId || stagedBankStatementLineId,
            documentId,
          ...
          createdOtherIncomeTransactions.push({ id: lineId, amount: lineAmount });
        }
        await markStagedBankLineMatched(createdOtherIncomeTransactions.map((transaction) => transaction.id), createdOtherIncomeTransactions);
      ...
      await markStagedBankLineMatched([id], [{ id, amount }]);
  ...
  return <CreatorProfilesContext.Provider value={creatorProfiles}><FreeCreatorSampleCodesContext.Provider value={freeCreatorSampleCodes}><main className="app-shell">
    ...
    <section className="main-area">
      ...
      {workspace === "accounting" && session.role === "admin" && <AccountingWorkspacePage
        ...
        onStageBankStatementLine={stageBankStatementLine}
        onUseBankStatementLine={useBankStatementLineForTransaction}
        onPairInternalTransferLine={pairInternalTransferLine}
...
function bankLineRawString(line: AccountingBankStatementLine, key: string) {
  const value = line.rawData?.[key];
  return typeof value === "string" ? value : "";
}

function bankLineWorkflow(line: AccountingBankStatementLine) {
  return bankLineRawString(line, "bank_workflow");
}

function bankLineStagedEvent(line: AccountingBankStatementLine) {
  return bankLineRawString(line, "bank_staged_event") || line.suggestedEvent || "";
}

function bankLineStagedAccount(line: AccountingBankStatementLine) {
...
function AccountingWorkspacePage({
  ...
  onStageBankStatementLine,
  onUseBankStatementLine,
  onPairInternalTransferLine,
  ...
  onMatchBankStatementLine: (line: AccountingBankStatementLine, formOverride?: Partial<BankStatementMatchForm>) => void;
  onStageBankStatementLine: (line: AccountingBankStatementLine, paymentMethod?: string, formOverride?: Partial<BankStatementMatchForm>) => void;
  ...
  onLinkBankStatementLine: (line: AccountingBankStatementLine, transaction: AccountingTransaction, correctedAmount?: number) => void;
  ...
  const [bankLineActions, setBankLineActions] = useState<Record<string, "match" | "new" | "pair" | "note" | "">>({});
  const [bankLineSelectedTransactions, setBankLineSelectedTransactions] = useState<Record<string, string>>({});
  const [bankLineMatchAmounts, setBankLineMatchAmounts] = useState<Record<string, string>>({});
  ...
  const [selectedBankStatementSection, setSelectedBankStatementSection] = useState<BankStatementSection>("unmatched");
  ...
  const shownBankLineIds = shownBankStatementLines.map((line) => line.id);
  const selectedShownBankLines = shownBankStatementLines.filter((line) => selectedBankLineIds.includes(line.id));
  const selectedActionableBankLines = selectedShownBankLines.filter((line) => line.matchStatus === "unmatched");
  const allShownBankLinesSelected = shownBankLineIds.length > 0 && shownBankLineIds.every((id) => selectedBankLineIds.includes(id));
  function toggleBankLineSelection(lineId: string, checked: boolean) {
    setSelectedBankLineIds((current) => checked ? [...new Set([...current, lineId])] : current.filter((id) => id !== lineId));
  }
  function toggleShownBankLineSelection(checked: boolean) {
    setSelectedBankLineIds((current) => checked ? [...new Set([...current, ...shownBankLineIds])] : current.filter((id) => !shownBankLineIds.includes(id)));
  }
  async function applyBulkBankStatementAction(action: "ads" | "drawings" | "shopee" | "sales" | "ignore" | "remove") {
    const rows = action === "remove" ? selectedShownBankLines : selectedActionableBankLines;
    if (!rows.length) return;
    if (action === "remove" && !confirm(`Remove ${rows.length} selected bank row${rows.length === 1 ? "" : "s"}?`)) return;
    for (const line of rows) {
      if (action === "ads" && line.moneyOut > 0) await onMatchBankStatementLine(line, { businessEvent: "marketing_expense", accountName: quickMarketingAccountForLine(line), notes: bankStatementNoteDrafts[line.id] ?? line.notes ?? "" });
      if (action === "drawings" && line.moneyOut > 0) await onMatchBankStatementLine(line, { businessEvent: "owner_drawings", accountName: "Owner Drawings", notes: bankStatementNoteDrafts[line.id] ?? line.notes ?? "" });
      if (action === "shopee") await onStageBankStatementLine(line, "Shopee Pay");
      if (action === "sales" && line.moneyIn > 0) await onMarkBankStatementSales(line);
      if (action === "ignore") await onIgnoreBankStatementLine(line);
      if (action === "remove") await onDeleteBankStatementLine(line);
    }
    setSelectedBankLineIds((current) => current.filter((id) => !rows.some((line) => line.id === id)));
  }
  ...
  function quickMarketingAccountForLine(line: AccountingBankStatementLine) {
    const description = line.description.toLowerCase();
    const options = bankStatementAccountOptions("marketing_expense");
    const preferred = description.includes("tiktok")
      ? options.find((option) => option.toLowerCase().includes("tiktok"))
      : description.includes("facebk") || description.includes("facebook") || description.includes("meta")
        ? options.find((option) => option.toLowerCase().includes("meta"))
        : "";
    return preferred || options[0] || "Meta ads";
  }
  ...
  function candidateInternalTransfersForBankLine(line: AccountingBankStatementLine) {
    const lineDate = Date.parse(line.transactionDate);
    const amount = getBankLineAmount(line);
    if (!amount || Number.isNaN(lineDate)) return [];
    return bankStatementLines.filter((candidate) => {
      ...
      const candidateDate = Date.parse(candidate.transactionDate);
      const oppositeDirection = (line.moneyIn > 0 && candidate.moneyOut > 0) || (line.moneyOut > 0 && candidate.moneyIn > 0);
      const sameAmount = Math.abs(getBankLineAmount(candidate) - amount) < 0.005;
      const closeDate = !Number.isNaN(candidateDate) && Math.abs(candidateDate - lineDate) <= 24 * 60 * 60 * 1000;
      return oppositeDirection && sameAmount && closeDate;
    }).sort((a, b) => Math.abs(Date.parse(a.transactionDate) - lineDate) - Math.abs(Date.parse(b.transactionDate) - lineDate) || a.rowNumber - b.rowNumber);
  }
  ...
  const bankInProgressPanelFor = (eventValue: string) => {
    const rows = bankStatementInProgressLines
      .filter((line) => bankLineStagedEvent(line) === eventValue)
      .sort((a, b) => dateKey(a.transactionDate).localeCompare(dateKey(b.transactionDate)) || a.rowNumber - b.rowNumber);
    return <BankStatementInProgressPanel rows={rows} onUse={onUseBankStatementLine} />;
  };
  ...
  if (view === "accounting_operating_costs") return <section className="accounting-workspace">
    ...
    {bankInProgressPanelFor("operating_cost")}
  ...
  if (view === "accounting_bank_reconciliation") return <section className="accounting-workspace">
    ...
    <section className="csv-import-layout bank-statement-layout">
      ...
      <section className={`card accounting-table-card bank-statement-card ${bankStatementFocusMode ? "bank-statement-fullscreen" : ""} lock-${lockedBankStatementColumn}`}>
        ...
        <div className="bank-bulk-actions no-print">
          <label className="bank-select-all"><input type="checkbox" checked={allShownBankLinesSelected} disabled={!shownBankStatementLines.length} onChange={(event) => toggleShownBankLineSelection(event.target.checked)} /> Select shown</label>
          <span>{selectedShownBankLines.length} selected</span>
          <button className="view-button" disabled={saving || !selectedActionableBankLines.some((line) => line.moneyOut > 0)} onClick={() => applyBulkBankStatementAction("ads")}>Ads</button>
          <button className="view-button" disabled={saving || !selectedActionableBankLines.some((line) => line.moneyOut > 0)} onClick={() => applyBulkBankStatementAction("drawings")}>Drawings</button>
          <button className="view-button" disabled={saving || !selectedActionableBankLines.length} onClick={() => applyBulkBankStatementAction("shopee")}>Shopee</button>
          <button className="view-button" disabled={saving || !selectedActionableBankLines.some((line) => line.moneyIn > 0)} onClick={() => applyBulkBankStatementAction("sales")}>Sales</button>
          <button className="view-button" disabled={saving || !selectedActionableBankLines.length} onClick={() => applyBulkBankStatementAction("ignore")}>Ignore</button>
          <button className="view-button danger-text" disabled={saving || !selectedShownBankLines.length} onClick={() => applyBulkBankStatementAction("remove")}>Remove</button>
          <button className="view-button" disabled={!selectedShownBankLines.length} onClick={() => setSelectedBankLineIds((current) => current.filter((id) => !shownBankLineIds.includes(id)))}>Clear</button>
        </div>
        <div className="table-scroll bank-statement-lines-scroll"><table className="orders-table bank-statement-table"><thead><tr><th><input type="checkbox" checked={allShownBankLinesSelected} disabled={!shownBankStatementLines.length} onChange={(event) => toggleShownBankLineSelection(event.target.checked)} /></th><th>Status</th><th>Date</th><th>Bank</th><th>Description</th><th>Amount</th><th>Action</th></tr></thead><tbody>{shownBankStatementLines.map((line) => {
          ...
          const selectedFormAccount = options.includes(form.accountName) ? form.accountName : options[0] || "";
          const action = bankLineActions[line.id] || "";
          const candidates = candidateTransactionsForBankLine(line);
          const pairCandidates = candidateInternalTransfersForBankLine(line);
          const selectedPair = bankStatementLines.find((candidate) => candidate.id === bankLineSelectedPairs[line.id]);
          const selectedTransaction = transactions.find((transaction) => transaction.id === bankLineSelectedTransactions[line.id]);
          const selectedMatchAmount = bankLineMatchAmounts[line.id] ?? (selectedTransaction ? String(selectedTransaction.amount) : "");
          ...
          const statusLabel = bankLineStatusLabel(line, isPartialMatch);
          const statusClass = statusLabel.replace(/\s+/g, "-");
          const canOpenLinked = hasLinkedTransactions;
          const inProgress = bankLineSection(line) === "in_progress";
          ...
          <tr className={`bank-line-${line.matchStatus} ${canOpenLinked ? "clickable-bank-line" : ""}`} onClick={(event) => { if (!canOpenLinked || (event.target as HTMLElement).closest("button,a,input,textarea,select")) return; onOpenLinkedBankTransaction(line); }}>
            <td><input type="checkbox" checked={selectedBankLineIds.includes(line.id)} onChange={(event) => toggleBankLineSelection(line.id, event.target.checked)} onClick={(event) => event.stopPropagation()} /></td>
            <td><span className={`source-document-pill ${statusClass}`}>{statusLabel}</span></td>
            ...
            <td><div className="bank-row-actions">{inProgress ? <button className="view-button" disabled={saving} onClick={() => onUseBankStatementLine(line)}>Use row</button> : <><button className="view-button" disabled={saving || line.matchStatus !== "unmatched"} onClick={() => setBankLineActions((current) => ({ ...current, [line.id]: current[line.id] === "match" ? "" : "match" }))}>Match</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched"} onClick={() => setBankLineActions((current) => ({ ...current, [line.id]: current[line.id] === "new" ? "" : "new" }))}>New</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched" || line.moneyOut <= 0} onClick={() => onMatchBankStatementLine(line, { businessEvent: "marketing_expense", accountName: quickMarketingAccountForLine(line), notes: noteValue })}>Ads</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched" || line.moneyOut <= 0} onClick={() => onMatchBankStatementLine(line, { businessEvent: "owner_drawings", accountName: "Owner Drawings", notes: noteValue })}>Drawings</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched"} onClick={() => onStageBankStatementLine(line, "Shopee Pay")}>Shopee</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched"} onClick={() => setBankLineActions((current) => ({ ...current, [line.id]: current[line.id] === "pair" ? "" : "pair" }))}>Pair</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched" || line.moneyIn <= 0} onClick={() => onMarkBankStatementSales(line)}>Sales</button><button className="view-button" disabled={saving || line.matchStatus !== "unmatched"} onClick={() => onIgnoreBankStatementLine(line)}>Ignore</button></>}<button className="view-button" disabled={saving} onClick={() => setBankLineActions((current) => ({ ...current, [line.id]: current[line.id] === "note" ? "" : "note" }))}>Note</button><button className="view-button danger-text" disabled={saving} onClick={() => onDeleteBankStatementLine(line)}>Remove</button></div>{hasLinkedTransactions && <div className={`bank-match-counter ${remainingAmount > 0.01 ? "partial" : "balanced"}`}><span>{formatMoney(matchedTotal)} matched</span><strong>{remainingAmount > 0.01 ? `${formatMoney(remainingAmount)} left` : "Balanced"}</strong></div>}</td>
          </tr>
          {action === "note" && <tr className="bank-line-detail-row"><td colSpan={7}><section className="posting-preview bank-line-action-panel bank-line-note-panel"><h3>Bank row note</h3><p>Add a reminder or explanation for this statement row.</p><div className="bank-line-note"><textarea value={noteValue} onChange={(event) => onBankStatementNoteChange(line.id, event.target.value)} placeholder="Add note for this bank row..." /><button className="button primary" disabled={!noteChanged || saving} onClick={() => onSaveBankStatementNote(line)}>Save note</button></div></section></td></tr>}
          {action === "match" && line.matchStatus === "unmatched" && <tr className="bank-line-detail-row"><td colSpan={7}><section className="posting-preview bank-line-action-panel"><h3>Match existing transaction</h3><p>Showing unlinked bookkeeping transactions within +/- 3 days of this bank row. You can correct the selected transaction amount before matching.</p><div className="bank-match-summary"><span>Bank amount {formatMoney(getBankLineAmount(line))}</span><span>Matched {formatMoney(matchedTotal)}</span><strong>{remainingAmount > 0.01 ? `${formatMoney(remainingAmount)} left` : "Balanced"}</strong></div>{matchedTransactions.length > 0 && <div className="bank-linked-list">{matchedTransactions.map((transaction) => <div key={transaction.id}><span>{formatDate(transaction.transactionDate)}</span><strong>{transaction.description}</strong><em>{formatMoney(transaction.amount)}</em></div>)}</div>}<div className="bank-match-edit-grid"><label>Existing transaction<select value={bankLineSelectedTransactions[line.id] || ""} onChange={(event) => { const transactionId = event.target.value; const nextTransaction = transactions.find((item) => item.id === transactionId); setBankLineSelectedTransactions((current) => ({ ...current, [line.id]: transactionId })); setBankLineMatchAmounts((current) => ({ ...current, [line.id]: nextTransaction ? String(nextTransaction.amount) : "" })); }}><option value="">Choose transaction</option>{candidates.map((transaction) => <option key={transaction.id} value={transaction.id}>{formatDate(transaction.transactionDate)} | {transaction.description} | {displayAccountingAccountName(transaction.accountName)} | {formatMoney(transaction.amount)}</option>)}</select></label><label>Amount to match / update<input type="number" min="0" step="0.01" value={selectedMatchAmount} disabled={!selectedTransaction} onChange={(event) => setBankLineMatchAmounts((current) => ({ ...current, [line.id]: event.target.value }))} placeholder={selectedTransaction ? String(selectedTransaction.amount) : "Choose transaction first"} /></label></div>{selectedTransaction && Math.abs((Number(selectedMatchAmount) || 0) - selectedTransaction.amount) >= 0.005 && <p className="bank-match-edit-note">This will update the selected transaction from {formatMoney(selectedTransaction.amount)} to {formatMoney(Number(selectedMatchAmount) || 0)} before matching.</p>}<div className="csv-import-actions"><button className="button primary" disabled={!selectedTransaction || saving || !(Number(selectedMatchAmount) > 0)} onClick={() => selectedTransaction && onLinkBankStatementLine(line, selectedTransaction, Number(selectedMatchAmount))}>Match and update amount</button>{!candidates.length && <span>No unlinked transactions found within +/- 3 days.</span>}</div></section></td></tr>}
  ...
  if (categoryEvent) {
    ...
    const processorBalance = transactionForm.categoryId === "Stripe"
      ...
      : transactionForm.categoryId === "Xendit"
        ? xenditBalance
        : transactionForm.categoryId === "TikTok"
          ? 0
    ...
    if (isMoneyIn) return <section className="accounting-workspace">
      <div className="accounting-hero card"><div><p>CASH</p><h2>Payment processor payouts</h2><span>Collection totals come from the fulfilment sales report. When Stripe, Xendit, or TikTok pays out, record the payout here: debit Bank, credit the payment processor.</span></div><div className="accounting-status-pill">{formatMoney(sales.totalCollected)}</div></div>
      <section className="sales-stats">
      ...
      {bankInProgressPanelFor("payment_processor_paid")}
      <section className="accounting-form-grid">
        <div className="accounting-form card">
          ...
          <label>{transactionForm.categoryId === "Stripe" || transactionForm.categoryId === "Xendit" || transactionForm.categoryId === "TikTok" ? "Net amount received" : transactionForm.categoryId === "Drawings" ? "Amount withdrawn" : "Amount received"}<input type="number" min="0" step="0.01" value={transactionForm.amount} onChange={(input) => onTransactionFormChange({ amount: input.target.value })} /></label>
    ...
    return <section className="accounting-workspace">
      ...
      {bankInProgressPanelFor(categoryEvent.value)}
      <section className="accounting-form-grid">
        <div className="accounting-form card">
...
function BankStatementInProgressPanel({ rows, onUse }: { rows: AccountingBankStatementLine[]; onUse: (line: AccountingBankStatementLine) => void }) {
  if (!rows.length) return null;
  return <section className="card bank-in-progress-panel">
    <div className="ledger-table-heading">
      <div><h3>In progress bank rows</h3><p>Click a row to load its date, description, amount, and bank details into the form below.</p></div>
      <span className="accounting-status-pill">{rows.length} pending</span>
    </div>
    <div className="bank-in-progress-list">
      {rows.map((line) => <button key={line.id} type="button" className="bank-in-progress-item" onClick={() => onUse(line)}>
        <span>{formatDate(line.transactionDate)}<small>{bankLineRawString(line, "bank") || bankLineRawString(line, "bank_account") || "Bank"}</small></span>
        <strong>{line.description}</strong>
        <em className={line.moneyIn > 0 ? "money-in" : "money-out"}>{formatMoney(getBankLineAmount(line))}</em>
        <span className="source-document-pill in-progress">Use row</span>
      </button>)}
    </div>
  </section>;
}
...
function PreordersWorkspacePage() {
  ...
  const speakerPrices: Record<string, number> = { plushie_5s: 115, plushie_10s: 125, plushie_20s: 135 };
  const productPrice = speakerPrices[form.productKey] ?? 115;
  const shippingPrice = form.shippingRegion === "EAST" ? 30 : 0;
  ...
  return <section className="manual-orders-workspace">
    <div className="manual-order-hero card"><div><p>PREORDERS</p><h2>Record a customer deposit</h2><span>Use the same customer and product details as Manual Orders, then add the deposit, balance due date, and payment proof.</span></div><div className="accounting-status-pill">{orders.length} preorder{orders.length === 1 ? "" : "s"}</div></div>
    {notice && <div className="notice"><span>{notice}</span></div>}
    <div className="manual-order-grid"><form className="card manual-order-form" onSubmit={submit}>
      <h3>Create preorder</h3>
      <label>Customer name<input required placeholder="Sarah Lim" value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })}/></label>
      <label>Phone<input required placeholder="0123456789" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })}/></label>
      <div className="manual-order-choice"><span>Character</span><div className="manual-order-pill-group">{manualOrderCharacters.map((character) => <button type="button" key={character} className={form.character === character ? "active" : ""} onClick={() => setForm({ ...form, character })}>{character}</button>)}</div></div>
      <div className="manual-order-choice"><span>Speaker</span><div className="manual-order-pill-group">{manualOrderProducts.map((product) => <button type="button" key={product.key} className={selectedProduct.key === product.key ? "active" : ""} onClick={() => setForm({ ...form, productKey: product.key })}>{speakerLabel(product.displayName)}</button>)}</div></div>
      <label>Shipping region<select value={form.shippingRegion} onChange={(e) => setForm({ ...form, shippingRegion: e.target.value === "EAST" ? "EAST" : "WEST" })}><option value="WEST">West Malaysia</option><option value="EAST">East Malaysia</option></select></label>
      <div className="manual-order-price-breakdown"><span>Normal order price</span><strong>RM {productPrice.toFixed(2)}</strong><small>{selectedProduct.displayName}</small><span>Shipping</span><strong>{shippingPrice ? `RM ${shippingPrice.toFixed(2)}` : "Free"}</strong><small>{form.shippingRegion === "EAST" ? "East Malaysia" : "West Malaysia"}</small><span>Total preorder price</span><strong>RM {totalPrice.toFixed(2)}</strong></div>
      <label>Deposit received (RM)<input required type="number" min="0" step="0.01" value={form.depositAmount} onChange={(e) => setForm({ ...form, depositAmount: e.target.value })}/></label>
      <strong>Balance remaining: RM {balance.toFixed(2)}</strong>
      <label>Balance payment date<input type="date" value={form.balanceDueDate} onChange={(e) => setForm({ ...form, balanceDueDate: e.target.value })}/></label>
      <label className="file-drop-zone" onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); setFiles(Array.from(e.dataTransfer.files)); }}>Drag payment PDFs/images here, or choose files<input type="file" multiple accept="application/pdf,image/png,image/jpeg,image/webp" onChange={(e) => setFiles(Array.from(e.target.files || []))}/></label>
      {files.length > 0 && <small>{files.length} file{files.length === 1 ? "" : "s"} attached: {files.map((file) => file.name).join(", ")}</small>}
      <button className="button primary large">Save preorder</button>
    </form><section className="card manual-order-result"><div className="accounting-form-heading"><div><h3>Preorder details</h3><p>Payment proof stays attached to this preorder.</p></div></div><div className="empty compact"><strong>Waiting for balance payment</strong><p>When the balance is fully paid, choose Mark paid in the preorder records below. A Manual Order checkout link will then be created automatically.</p></div></section></div>
    <section className="card accounting-table-card manual-order-table-card"><div className="manual-order-table-toolbar"><div><h3>Preorder records</h3><p>Payment receipts remain attached to the preorder and its generated Manual Order.</p></div></div><div className="table-scroll"><table className="orders-table"><thead><tr><th>Customer</th><th>Product</th><th>Deposit</th><th>Balance</th><th>Due</th><th>Receipts</th><th>Status</th><th /></tr></thead><tbody>{orders.map((order) => <tr key={order.id}><td><strong>{order.customer_name}</strong><small>{order.phone}</small></td><td>{order.character} · {speakerLabel(manualOrderProducts.find((p) => p.key === order.product_key)?.displayName || order.product_key)}</td><td>RM {Number(order.deposit_amount).toFixed(2)}</td><td>RM {(Number(order.total_amount) - Number(order.deposit_amount)).toFixed(2)}</td><td>{order.balance_due_date || "-"}</td><td>{(order.payment_receipts || []).map((receipt) => <a key={receipt.url} href={receipt.url} target="_blank" rel="noreferrer">{receipt.fileName} </a>) || "-"}</td><td>{order.status.replace(/_/g, " ")}</td><td>{order.status === "deposit_paid" && <button className="button primary small" onClick={() => void markPaid(order)}>Mark paid & create Manual Order</button>}</td></tr>)}{!orders.length && <tr><td colSpan={8}>No preorders yet.</td></tr>}</tbody></table></div></section>
  </section>;
}
...
function OrderDrawer({ order, manualOrders, role, actor, onClose, onUpdate, onStatus }: { order: Order; manualOrders: ManualOrder[]; role: UserRole; actor: string; onClose: () => void; onUpdate: (patch: Partial<Order>) => void; onStatus: (status: OrderStatus) => void }) {
  ...
  const fulfilmentChannel = fulfilmentSource(order, manualOrders);
  const source = fulfilmentSourceLabel(order, manualOrders);
  const isCod = isCodFulfilmentOrder(order, manualOrders);
  const paymentMethod = fulfilmentChannel === "tiktok" ? "Collected by TikTok Shop" : isCod ? "Cash on delivery (COD)" : order.paymentProcessor || "Unknown";
  const missingTikTokAttachment = tikTokAttachmentNeedsReupload(order);
